package com.ankamagames.dofus.logic.game.fight.miscs
{
   ...
   public class PushUtil
   {
      ...
      public static function getPushedEntities(param1:SpellWrapper, param2:int, param3:int) : Vector.<PushedEntity>
      {
         ...
         _loc12_ = !DamageUtil.hasMinSize(_loc7_)?uint(param2):uint(param3);
      ...
      private static function getPushedEntitiesInLine(param1:SpellWrapper, param2:Boolean, param3:EffectInstance, param4:int, param5:int, param6:int, param7:int) : Vector.<PushedEntity>
      {
         ...
         var getPushedEntity:Function = function(param1:Number):PushedEntity
         {
         ...
         var isEntityInSpellZone:Function = function(param1:Number):Boolean
         {
         ...
         while(i < nbEntities)
         {
            ...
            if(entityPushable && DamageUtil.verifySpellEffectZone(entities[i].id,pPushEffect,pSpellImpactCell,casterInfos.disposition.cellId) && DamageUtil.verifySpellEffectMask(pSpell.playerId,entities[i].id,pPushEffect,pSpellImpactCell))
            {
               ...
               if(!pushingEntity)
               {
                  pushingEntity = new PushedEntity(entities[i].id,pushedIndex,pPushForce,pPushEffect);
                  ...
                  if(nextCell)
                  {
                     if(isBlockingCell(nextCell.cellId,previousCell.cellId))
                     {
                        ...
                        if(nextCellEntity)
                        {
                           ...
                           if(!pushedEntity)
                           {
                              pushedEntity = new PushedEntity(nextCellEntity.id,pushedIndex,pPushForce,pPushEffect);
      ...
      private static function getPushForce(param1:int, param2:GameFightFighterInformations, param3:Vector.<EffectInstance>, param4:EffectInstance) : int
      {
         ...
         _loc11_ = MapPoint.fromCellId(param2.disposition.cellId);
         _loc12_ = MapPoint.fromCellId(param1);
         _loc10_ = (_loc11_.advancedOrientationTo(_loc12_,false) & 1) == 0?int(Math.ceil(_loc10_ / 2)):int(_loc10_);
         if(_loc7_ != -1 && _loc7_ < _loc6_ && isPushableEntity(param2))
         {
            _loc13_ = _loc8_.diceNum;
            _loc14_ = _loc11_;
            _loc16_ = _loc11_.advancedOrientationTo(_loc12_);
            _loc18_ = 0;
            _loc17_ = 0;
      ...
      public static function hasPushDamages(param1:Number, param2:Number, param3:Vector.<EffectInstance>, param4:EffectInstance, param5:int) : Boolean
      {
         ...
         if(_loc7_ && isPushableEntity(_loc7_))
         {
            ...
            _loc9_ = !DamageUtil.hasMinSize(param4.zoneShape)?uint(_loc8_.disposition.cellId):uint(param5);
            ...
            while(_loc17_ < _loc12_)
            {
               if(_loc15_)
               {
                  if(isBlockingCell(_loc15_.cellId,!_loc14_?int(_loc13_.cellId):int(_loc14_.cellId)))
                  {
                     break;
                  }
                  _loc16_--;
