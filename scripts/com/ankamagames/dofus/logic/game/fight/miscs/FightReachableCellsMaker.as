package com.ankamagames.dofus.logic.game.fight.miscs
{
   ...
   public class FightReachableCellsMaker
   {
      ...
      public function FightReachableCellsMaker(param1:GameFightFighterInformations, param2:int = -1, param3:int = -1)
      {
         ...
         var _loc10_:_ReachableCellStore = null;
         ...
         _loc8_ = EntitiesManager.getInstance().entities;
         for each(_loc9_ in _loc8_)
         {
            if(_loc9_.id != this._infos.contextualId && _loc9_.position)
            {
               _loc6_ = _loc9_.position.x - this._mapPoint.x + this._mp;
               _loc7_ = _loc9_.position.y - this._mapPoint.y + this._mp;
               if(_loc6_ >= 0 && _loc6_ < this._mp * 2 + 1 && _loc7_ >= 0 && _loc7_ < this._mp * 2 + 1)
               {
                  param1 = _loc4_.getEntityInfos(_loc9_.id) as GameFightFighterInformations;
                  if(param1)
                  {
                     ...
                     {
                        _loc10_ = new _ReachableCellStore(_loc9_.position,_loc6_,_loc7_,this._cellGrid);
                        _loc10_.state = _ReachableCellStore.STATE_UNREACHABLE;
                        _loc11_ = TackleUtil.getTackleForFighter(param1,this._infos);
                        if(!_loc10_.evade || _loc11_ < _loc10_.evade)
                        {
                           _loc10_.evade = _loc11_;
                        }
                        this._cellGrid[_loc6_][_loc7_] = _loc10_;
                     }
                  }
               }
            }
