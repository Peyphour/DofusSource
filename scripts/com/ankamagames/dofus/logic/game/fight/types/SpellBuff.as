   ...
   public class SpellBuff extends BasicBuff
   {
      ...
      override public function onApplyed() : void
      {
         ...
         if(_loc1_)
         {
                  ...
                  if(_loc3_.modificationType == this.modifType)
                  {
                     ...
                     if(stack)
                     {
                        for each(_loc5_ in stack)
                        {
                           ...
                           {
                              _loc3_.value.contextModif = _loc3_.value.contextModif + (_loc5_ as SpellBuff).delta;
                           }
                        }
                     }
                     else
            ...
            _loc4_ = SpellWrapper.getSpellWrapperById(this.spellId,targetId);
            _loc4_ = SpellWrapper.create(_loc4_.spellId,_loc4_.spellLevel,true,targetId);
            _loc4_.versionNum++;
         }
         super.onApplyed();
      }
      ...
      override public function onRemoved() : void
      {
         ...
         if(!_removed)
         {
            ...
            if(_loc1_)
            {
               for each(_loc2_ in _loc1_.spellModifications)
               {
                  if(this.spellId == _loc2_.spellId)
                  {
                     if(_loc2_.modificationType == this.modifType)
                     {
                        if(stack)
                        {
                           for each(_loc4_ in stack)
                           {
                              if(_loc4_ is SpellBuff)
                              {
                                 _loc2_.value.contextModif = _loc2_.value.contextModif - (_loc4_ as SpellBuff).delta;
                              }
                           }
                        }
                        else
               ...
               _loc3_ = SpellWrapper.getSpellWrapperById(this.spellId,targetId);
               _loc3_ = SpellWrapper.create(_loc3_.spellId,_loc3_.spellLevel,true,targetId);
               _loc3_.versionNum++;
            }
         }
         super.onRemoved();
