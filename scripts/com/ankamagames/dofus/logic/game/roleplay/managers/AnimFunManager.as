package com.ankamagames.dofus.logic.game.roleplay.managers
{
   ...
   public final class AnimFunManager
   {
      ...
      public function cancelAnim(param1:Number) : void
      {
      ...
      private function checkAvailableAnim(param1:SynchroTimer) : void
      {
         ...
         var _loc9_:RoleplayEntitiesFrame = null;
         ...
         if((!this._firstAnim || this._firstAnim && !this._lastAnim) && (this._lastAnim != _loc2_ && (_loc2_.fastAnim && _loc2_ != this._lastAnimFast || !_loc2_.fastAnim && _loc2_ != this._lastAnimNormal)))
         {
            ...
            if(_loc2_)
            {
               ...
               {
                  _loc8_ = DofusEntities.getEntity(_loc2_.actorId) as AnimatedCharacter;
                  if(!_loc8_)
                  {
                     return;
                  }
                  _loc9_ = Kernel.getWorker().getFrame(RoleplayEntitiesFrame) as RoleplayEntitiesFrame;
                  if(!_loc9_)
                  {
                     return;
                  }
                  _loc10_ = MapDisplayManager.getInstance().getDataMapContainer().dataMap.cells[_loc8_.position.cellId];
                  if(!Atouin.getInstance().options.transparentOverlayMode && !_loc10_.visible)
                  {
                     return;
                  }
                  if(_loc9_.hasIcon(_loc2_.actorId))
                  {
                     _loc9_.forceIconUpdate(_loc2_.actorId);
                  }
      ...
      private function randomAnim(param1:Number, param2:int) : String
      {
      ...
      private function getAnimSum(param1:AnimFun) : int
      {
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         var _loc4_:int = this._anims.length;
         _loc2_ = 0;
         while(_loc2_ < _loc4_)
         {
      ...
      private function hasFastAnims(param1:Number) : Boolean
      {
      ...
      private function hasAnimsFun(param1:Number) : Boolean
      {
         var _loc3_:Monster = null;
         var _loc4_:Npc = null;
         var _loc2_:GameContextActorInformations = (Kernel.getWorker().getFrame(RoleplayEntitiesFrame) as RoleplayEntitiesFrame).getEntityInfos(param1);
