package com.ankamagames.dofus.logic.game.common.types
{
   ...
   public class DofusShopArticle extends DofusShopObject implements IDataCenter
   {
      ...
      override public function init(param1:Object) : void
      {
         ...
         this._gids = [];
         for each(_loc6_ in param1.references)
         {
            if(_loc6_ && typeof _loc6_ == "object")
            {
               if(!_loc6_.type || _loc6_.type && _loc6_.type.toUpperCase() != "VIRTUALGIFT" || !_loc6_.content)
               {
                  this._references.push(new DofusShopArticleReference(_loc6_));
               }
               else if(_loc6_.content)
               {
                  this._references.push(_loc6_);
