package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   import com.ankamagames.dofus.network.messages.game.context.roleplay.fight.arena.GameRolePlayArenaUpdatePlayerInfosWithTeamMessage;
   ...
   public class PartyManagementFrame implements Frame
   {
      ...
      public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
               ...
               else if(pumsg.partyId == this._partyId)
               {
                     ...
                     if(partyMemberWrapper.id == pumsg.memberInformations.id)
                     {
                        ...
                        if(pumsg.memberInformations.companions.length > 0)
                        {
                           for each(serverMemberComp in pumsg.memberInformations.companions)
                           {
                              ...
                              {
                                 partyCompanionWrapper = new PartyCompanionWrapper(pumsg.memberInformations.id,pumsg.memberInformations.name,serverMemberComp.companionGenericId,true,pumsg.memberInformations.level > ProtocolConstantsEnum.MAX_LEVEL?int(ProtocolConstantsEnum.MAX_LEVEL):int(pumsg.memberInformations.level),serverMemberComp.entityLook,serverMemberComp.lifePoints,serverMemberComp.maxLifePoints,serverMemberComp.initiative,serverMemberComp.prospecting,serverMemberComp.regenRate);
                  ...
                  if(!existingMember)
                  {
                     ...
                     if(pumsg.memberInformations.companions.length > 0)
                     {
                        ...
                        {
                           partyCompanionWrapper = new PartyCompanionWrapper(pumsg.memberInformations.id,pumsg.memberInformations.name,partyCompanionMemberInfo.companionGenericId,false,pumsg.memberInformations.level > ProtocolConstantsEnum.MAX_LEVEL?int(ProtocolConstantsEnum.MAX_LEVEL):int(pumsg.memberInformations.level),partyCompanionMemberInfo.entityLook,partyCompanionMemberInfo.lifePoints,partyCompanionMemberInfo.maxLifePoints,partyCompanionMemberInfo.initiative,partyCompanionMemberInfo.prospecting,partyCompanionMemberInfo.regenRate);
               ...
               else
               {
                  ...
                  for each(memberJoin in pjmsg.members)
                  {
                     ...
                     if(memberJoin.companions.length > 0)
                     {
                        ...
                        {
                           partyCompanionWrapper = new PartyCompanionWrapper(memberJoin.id,memberJoin.name,partyCompanionMemberInfo.companionGenericId,true,memberJoin.level > ProtocolConstantsEnum.MAX_LEVEL?int(ProtocolConstantsEnum.MAX_LEVEL):int(memberJoin.level),partyCompanionMemberInfo.entityLook,partyCompanionMemberInfo.lifePoints,partyCompanionMemberInfo.maxLifePoints,partyCompanionMemberInfo.initiative,partyCompanionMemberInfo.prospecting,partyCompanionMemberInfo.regenRate);
                           partyCompanionWrapper.index = partyCompanionMemberInfo.indexId;
            ...
            case msg is PartyFollowStatusUpdateMessage:
               ...
               if(pfsumsg.success)
               {
                  if(pfsumsg.followedId == 0 && !pfsumsg.isFollowed)
                  {
                     ...
                     {
                        KernelEventsManager.getInstance().processCallback(HookList.RemoveMapFlag,"flag_srv" + CompassTypeEnum.COMPASS_TYPE_PARTY + "_" + followingPlayerId,PlayedCharacterManager.getInstance().currentWorldMap.id);
            ...
            case msg is GameRolePlayArenaUpdatePlayerInfosMessage:
               ...
               if(msg is GameRolePlayArenaUpdatePlayerInfosWithTeamMessage)
               {
                  this._arenaRankGroupInfos = ArenaRankInfosWrapper.create((grpaupimsg as GameRolePlayArenaUpdatePlayerInfosWithTeamMessage).team);
               }
               KernelEventsManager.getInstance().processCallback(RoleplayHookList.ArenaUpdateRank,this._arenaRankSoloInfos,this._arenaRankGroupInfos);
               return true;
      ...
      private function createPartyFightNotification(param1:uint, param2:PartyFightInformationsData, param3:Number) : void
      {
      ...
      public function getGroupMemberById(param1:Number) : PartyMemberWrapper
      {
      ...
      private function createPartyPlayerContextMenu(param1:Number, param2:int) : Object
      {
