package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   import com.ankamagames.dofus.network.messages.game.context.roleplay.spell.SpellModifyFailureMessage;
   import com.ankamagames.dofus.network.messages.game.context.roleplay.spell.SpellModifyRequestMessage;
   import com.ankamagames.dofus.network.messages.game.context.roleplay.spell.SpellModifySuccessMessage;
   ...
   public class PlayedCharacterUpdatesFrame implements Frame
   {
      ...
      public function process(param1:Message) : Boolean
      {
         ...
         var spurmsg:SpellModifyRequestMessage = null;
         var susmsg:SpellModifySuccessMessage = null;
         var updatedSW:SpellWrapper = null;
         var updated:Boolean = false;
         var previousCooldown:int = 0;
         var sufmsg:SpellModifyFailureMessage = null;
         ...
         switch(true)
         {
            ...
            case msg is IncreaseSpellLevelAction:
               ...
               spurmsg = new SpellModifyRequestMessage();
               spurmsg.initSpellModifyRequestMessage(isla.spellId,isla.spellLevel);
               ConnectionsHandler.getConnection().send(spurmsg);
               return true;
            case msg is SpellModifySuccessMessage:
               if(!PlayedCharacterManager.getInstance().spellsInventory)
               {
                  return true;
               }
               susmsg = msg as SpellModifySuccessMessage;
               updated = false;
               previousCooldown = 0;
               for each(sw in PlayedCharacterManager.getInstance().spellsInventory)
               {
                  ...
                  {
                     oldSw = SpellWrapper.getSpellWrapperById(sw.id,sw.playerId);
                     if(oldSw)
                     {
                        previousCooldown = oldSw.actualCooldown;
                     }
                     updatedSW = SpellWrapper.create(sw.id,susmsg.spellLevel,true,sw.playerId);
                     break;
                  }
               }
               if(updatedSW == null)
               {
                  swn = SpellWrapper.create(susmsg.spellId,susmsg.spellLevel,true,PlayedCharacterManager.getInstance().id);
                  PlayedCharacterManager.getInstance().spellsInventory.push(swn);
                  KernelEventsManager.getInstance().processCallback(HookList.SpellListUpdate,PlayedCharacterManager.getInstance().spellsInventory);
               }
               else
               {
                  KernelEventsManager.getInstance().processCallback(HookList.SpellModifySuccess,updatedSW);
                  updatedSW.actualCooldown = previousCooldown;
               }
               return true;
            case msg is SpellModifyFailureMessage:
               sufmsg = msg as SpellModifyFailureMessage;
               KernelEventsManager.getInstance().processCallback(HookList.SpellModifyFail);
               return true;
            ...
            case msg is CharacterLevelUpMessage:
               ...
               switch(messageId)
               {
                  case CharacterLevelUpMessage.protocolId:
                     ...
                     caracPointEarned = 0;
                     healPointEarned = 0;
                     if(clumsg.newLevel <= ProtocolConstantsEnum.MAX_LEVEL)
                     {
                     ...
                     for each(swBreed in playerBreed.breedSpells)
                     {
                        ...
                        {
                           swrapper = SpellWrapper.create(swBreed.id,1,false);
                     ...
                     else
                     {
                        ...
                        if(newSpell.length > 0)
                        {
                           ...
                           returnedSpell = SpellWrapper.create(selectedSpell.spellId,1,false);
            ...
            case msg is CompassResetMessage:
               ...
               switch(crmsg.type)
               {
                  ...
                  case CompassTypeEnum.COMPASS_TYPE_PARTY:
                     ...
                     {
                        KernelEventsManager.getInstance().processCallback(HookList.RemoveMapFlag,name + "_" + followingPlayerId,PlayedCharacterManager.getInstance().currentWorldMap.id);
                     }
               ...
               switch(cumsg.type)
               {
                  case CompassTypeEnum.COMPASS_TYPE_PARTY:
                     ...
                     else
                     {
                        ...
                        if(active)
                        {
                           KernelEventsManager.getInstance().processCallback(HookList.AddMapFlag,name,legend,PlayedCharacterManager.getInstance().currentWorldMap.id,cumsg.coords.worldX,cumsg.coords.worldY,39423,false);
                        }
                        else
                        {
                           KernelEventsManager.getInstance().processCallback(HookList.RemoveMapFlag,name,PlayedCharacterManager.getInstance().currentWorldMap.id);
            ...
            case msg is HaapiApiKeyMessage:
               ...
               HaapiKeyManager.getInstance().saveApiKey(hakmsg.keyType,hakmsg.token);
               return true;
            default:
               return false;
         }
      ...
      public function updateCharacterStatsList(param1:CharacterCharacteristicsInformations) : void
      {
         var _loc7_:int = 0;
         var _loc8_:int = 0;
         ...
         if(_loc2_ && _loc2_.energyPoints > param1.energyPoints)
         {
            KernelEventsManager.getInstance().processCallback(TriggerHookList.PlayerIsDead);
         }
         if(param1.kamas != InventoryManager.getInstance().inventory.kamas)
         {
            InventoryManager.getInstance().inventory.kamas = param1.kamas;
         }
         ...
         var _loc4_:Vector.<CharacterSpellModification> = param1.spellModifications;
         var _loc5_:int = !!_loc3_?int(Math.max(_loc3_.length,_loc4_.length)):int(_loc4_.length);
         ...
         PlayedCharacterManager.getInstance().characteristics = param1;
         for each(_loc8_ in _loc6_)
         {
            _loc9_ = SpellWrapper.getSpellWrapperById(_loc8_,PlayedCharacterManager.getInstance().id);
            if(_loc9_)
            {
               _loc9_ = SpellWrapper.create(_loc9_.spellId,_loc9_.spellLevel,true,PlayedCharacterManager.getInstance().id);
               _loc9_.versionNum++;
            }
         }
