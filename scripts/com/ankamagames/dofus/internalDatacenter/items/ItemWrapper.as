package com.ankamagames.dofus.internalDatacenter.items
{
   ...
   public class ItemWrapper extends Item implements ISlotData, ICellZoneProvider, IDataCenter
   {
      ...
      public static const ITEM_TYPE_DRAGOTURKEY_CERTIFICATE:uint = 97;
      
      public static const ITEM_TYPE_MULDO_CERTIFICATE:uint = 196;
      ...
      private static const EQUIPMENT_SUPER_TYPES:Array = [1,2,3,4,5,7,8,10,11,12,13,23,25];
      ...
      public function get isCertificate() : Boolean
      {
         ...
         return _loc1_ && (_loc1_.typeId == ITEM_TYPE_DRAGOTURKEY_CERTIFICATE || _loc1_.typeId == ITEM_TYPE_MULDO_CERTIFICATE);
      }
         ...
         if(!this._searchContent)
         {
            ...
            switch(this.objectGID)
            {
               ...
               case OBJECT_GID_SOULSTONE_MINIBOSS:
                  for each(_loc1_ in this.effectsList)
                  {
                     if(_loc1_.actionId == ActionIdConverter.ACTION_CHARACTER_SUMMON_MONSTER_GROUP)
                     {
                        _loc2_ = Monster.getMonsterById(_loc1_.diceConst);
                        if(_loc2_)
                        {
                           this._searchContent = this._searchContent + _loc2_.undiatricalName;
                        }
                     }
                  }
            }
      ...
      public function getIconUri(param1:Boolean = true) : Uri
      {
         ...
         if(this.isLivingObject)
         {
            _loc3_ = LivingObjectSkinJntMood.getLivingObjectSkin(!!this.livingObjectId?int(this.livingObjectId):int(this.objectGID),this.livingObjectMood,this.livingObjectSkin).toString();
         }
      ...
      private function updateEffects(param1:Vector.<ObjectEffect>) : void
      {
         ...
         {
            _loc7_ = ObjectEffectAdapter.fromNetwork(_loc6_);
            if(_loc3_ && _loc7_.category == 2)
            {
               _loc7_.zoneShape = _loc3_;
               _loc7_.zoneSize = _loc4_;
            }
            this.effects.push(_loc7_);
            this.updateLivingObjects(_loc7_);
            this.updatePresets(_loc7_);
            if(_loc5_ != -1 && (_loc7_.effectId == ActionIdConverter.ACTION_CHARACTER_ENERGY_POINTS_WIN || _loc7_.effectId == ActionIdConverter.ACTION_CHARACTER_BOOST_LIFE_POINTS || _loc7_.effectId == ActionIdConverter.ACTION_CHARACTER_GAIN_XP || _loc7_.effectId == ActionIdConverter.ACTION_CHARACTER_INVENTORY_GAIN_KAMAS || _loc7_.effectId == ActionIdConverter.ACTION_CHARACTER_INVENTORY_ADD_ITEM_NOCHECK))
            {
               _loc5_ = 1;
            }
            if(_loc5_ != -1 && _loc7_.effectId != ActionIdConverter.ACTION_CHARACTER_ENERGY_POINTS_WIN && _loc7_.effectId != ActionIdConverter.ACTION_CHARACTER_BOOST_LIFE_POINTS && _loc7_.effectId != ActionIdConverter.ACTION_CHARACTER_GAIN_XP && _loc7_.effectId != ActionIdConverter.ACTION_CHARACTER_INVENTORY_GAIN_KAMAS && _loc7_.effectId != ActionIdConverter.ACTION_CHARACTER_INVENTORY_ADD_ITEM_NOCHECK)
            ...
            if(_loc7_.effectId == 983)
            {
               this.exchangeAllowed = false;
            }
            if(_loc7_.effectId == 981 || _loc7_.effectId == 982)
            {
               exchangeable = false;
            }
            if(_loc7_.effectId == ACTION_ID_WRAPPER_OBJECT_CATEGORY)
            {
               this.wrapperObjectCategory = EffectInstanceInteger(_loc7_).value;
            }
         }
         if(_loc5_ == 1)
         {
