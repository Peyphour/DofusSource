   ...
   public class ConnectionsHandler
   {
      ...
      public static function onConnectionTimeout(param1:TimerEvent) : void
      {
         ...
         if(_currentConnection && _currentConnection.connected)
         {
            ...
            _currentConnection.send(_loc2_,_currentConnectionType);
            if(_connectionTimeout)
            {
               _connectionTimeout.stop();
            }
      ...
      public static function closeConnection() : void
      {
         ...
         var eventList:XMLList = DescribeTypeCache.typeDescription(_currentConnection)..metadata.(@name == "Event").arg.(@key == "").@value;
         for each(e in eventList)
         {
            _currentConnection.removeEventListener(e.toString(),onConnectionEvent);
         }
      ...
      private static function etablishConnection(param1:String, param2:int, param3:String, param4:Boolean = false, param5:IConnectionProxy = null) : void
      {
         ...
         if(param4)
         {
            ...
            _loc6_ = new SnifferServerConnection(null,0,param3);
         }
         else if(param5 != null)
         {
            _loc6_ = new ProxyedServerConnection(param5,null,0,param3);
         }
         else
         {
            _loc6_ = new ServerConnection(null,0,param3);
         }
         if(!_currentConnection)
         {
            createConnection();
      ...
      private static function createConnection() : void
      {
         ...
         eventList = DescribeTypeCache.typeDescription(_currentConnection)..metadata.(@name == "Event").arg.(@key == "").@value;
         for each(e in eventList)
         {
            _currentConnection.addEventListener(e.toString(),onConnectionEvent);
         }
      }
      
      private static function onConnectionEvent(param1:Event) : void
      {
      }
   }
}
