      ...
      private function createParticle(param1:DisplayObjectContainer, param2:uint, param3:Array, param4:uint, param5:Number, param6:Array, param7:Function, param8:Number = 0, param9:Number = 0) : void
      {
         ...
         while(_loc10_ < param2)
         {
            if(CURRENT_PARTICLES < MAX_PARTICLES)
            {
               ...
               if(param4 == TYPE_MIX)
               {
                  _loc12_ = Math.random() > 0.5?uint(TYPE_TWIRL):uint(TYPE_CLASSIC);
               }
               if(param3)
               {
                  if(param4 == TYPE_MIX)
               ...
               _particules[_loc11_] = new TwirlParticle(_loc11_,100,Math.random() < param5,param7,-param1.parent.y + 20 * Math.random() - 10,_loc12_ == TYPE_TWIRL?uint(10):uint(0));
               CURRENT_PARTICLES++;
            }
            _loc10_++;
         }
