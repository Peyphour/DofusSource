package com.ankamagames.dofus.console.moduleLUA
{
   ...
   public class ConsoleLUA
   {
      ...
      private function loadScript(param1:File) : void
      {
         this.clearConsole();
         if(!param1)
         {
         ...
         if(this._file == param1)
         {
            this.log("Reloading script " + param1.name);
         }
         else
         {
            this.log("Loading script " + param1.name);
         }
         var _loc2_:FileStream = new FileStream();
         _loc2_.open(param1,FileMode.READ);
         this._loadedScript = _loc2_.readUTFBytes(_loc2_.bytesAvailable);
         if(this._frame)
         {
            this._frame.luaScript = this._loadedScript;
         }
         _loc2_.close();
         this.log("Script \'" + param1.name + "\' loaded.");
         this.printLine(this._loadedScript);
      }
      
      protected function reloadScript(param1:MouseEvent) : void
      {
         this.loadScript(this._file);
      }
      
      protected function loadScriptFromFile(param1:MouseEvent) : void
      {
         var file:File = null;
         var event:MouseEvent = param1;
      ...
      private function logScript(param1:String) : void
      {
         this.printLine("############################");
         this.printLine("########## Script ##########");
         this.printLine("############################");
         this.printLine(param1);
         this.printLine("############################");
      }
      
      private function log(param1:String) : void
      {
         this.printLine("<span class=\'gray\'>> " + param1 + "</span>",false);
      }
      ...
      public function printLine(param1:String, param2:Boolean = true) : void
      {
         if(param2)
         {
            param1 = param1.replace(/\r/gi,"");
            param1 = param1.replace(/(".*")/g,"<span class=\'string\'>$&</span>");
            param1 = param1.replace(/local/g,"<span class=\'local\'>local</span>");
            param1 = param1.replace(/(?<![a-z])([+-]?[0-9]+(?:\.[0-9]+)?)/g,"<span class=\'number\'>$&</span>");
            param1 = param1.replace(/(?<=[\.])[^.(]+(?=[(])/g,"<span class=\'function\'>$&</span>");
            param1 = param1.replace(/\(/g,"<span class=\'function\'>$&</span>");
            param1 = param1.replace(/\)/g,"<span class=\'function\'>$&</span>");
         }
