package com.ankamagames.dofus.console.debug
{
   ...
   public class DisplayMapInstructionHandler implements ConsoleInstructionHandler
   {
      ...
      {
         var proto169Frame:Proto169Frame = null;
         ...
         switch(cmd)
         {
            case "enable169background":
               MapRenderer.PROTO_169_BACKGROUND = !MapRenderer.PROTO_169_BACKGROUND;
            case "enable169":
               proto169Frame = Kernel.getWorker().getFrame(Proto169Frame) as Proto169Frame;
               if(proto169Frame == null)
               {
                  Kernel.getWorker().addFrame(new Proto169Frame());
               }
               else
               {
                  Kernel.getWorker().removeFrame(proto169Frame);
               }
               break;
            ...
            case "tutofx":
               ...
               ts.addEventListener(TiphonEvent.RENDER_SUCCEED,function(param1:Event):void
               {
                  ts.stopAnimation(1);
               });
               ...
               break;
            case "debugwidescreen":
               if(!MapRenderer.PROTO_169_BACKGROUND)
               {
                  DataGroundMapManager.clearGroundCache();
                  Atouin.getInstance().setWorldMaskDimensions(AtouinConstants.WIDESCREEN_BITMAP_WIDTH);
               }
               else
               {
                  Atouin.getInstance().setWorldMaskDimensions(StageShareManager.startWidth);
               }
               this.handle(console,"enable169background",null);
               break;
            case "setfrustum":
            ...
            case "setclientwindowsize":
               ...
               {
                  console.output("This command requires an AIR Dofus client");
               }
               break;
      ...
      public function getHelp(param1:String) : String
      {
         ...
         {
            case "enable169":
               return "Active le mode 16/9eme";
            case "enable169background":
               return "Active le mode 16/9eme avec le background rendu par le client";
            case "displaymapdebug":
