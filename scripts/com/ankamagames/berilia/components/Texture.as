package com.ankamagames.berilia.components
{
   ...
   public class Texture extends TextureBase implements FinalizableUIComponent, IRectangle
   {
      ...
      {
         if(param1 != _uri || this._forceReload)
         {
            _uri = param1;
            if(_finalized)
            {
      ...
      {
         this._bitmap = new Bitmap(param1,"auto",_smooth);
         if(_finalized)
         {
            this.reload();
         }
      ...
      private function reload() : void
      {
         ...
         else if(_uri)
         {
            ...
            if(_uri.subPath)
            {
               ...
               {
                  _loc1_ = new Uri(_uri.normalizedUri);
               }
               else if(AirScanner.hasAir() && _uri.protocol != "httpc")
               {
                  _loc1_ = new Uri(_uri.path);
               }
               else
               {
                  _loc1_ = _uri;
               }
               _loc1_.loaderContext = _uri.loaderContext;
            ...
            if(_uri.protocol == "httpc" && this.defaultBitmapData && !this._loader.isInCache(uri))
            {
            ...
            if(_loc1_.fileType.toLowerCase() != "swf")
            {
               _useCache = false;
            }
            else
            ...
            this._loader.load(_loc1_,!!_useCache?UriCacheFactory.getCacheFromUri(_loc1_):null,_loc2_,_loc3_);
         }
         else
         {
            if(this._child)
      ...
      override protected function onFailed(param1:ResourceErrorEvent) : void
      {
         ...
         {
            this._log.error("UI " + (!!getUi()?getUi().name:"unknow") + "/" + name + ", texture resource not found: " + (!!param1?param1.errorMsg:"No ressource specified.") + ", requested uri : " + param1.uri);
         }
         dispatchEvent(new TextureLoadFailedEvent(this,_loc2_));
         Berilia.getInstance().handler.process(new TextureLoadFailMessage(this));
         if(!_loc2_.cancel && param1.uri == _uri)
         {
            this._loader = null;
            if(param1.uri == _uri)
            {
               ...
               if(_showLoadingError)
               {
                  ...
                  _loc3_.graphics.drawRect(0,0,!isNaN(this._forcedWidth) && this._forcedWidth != 0?Number(this._forcedWidth):Number(10),!isNaN(this._forcedHeight) && this._forcedHeight != 0?Number(this._forcedHeight):Number(10));
