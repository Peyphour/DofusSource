   ...
   public class UIInteractionFrame implements Frame
   {
      ...
      public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case param1 is HumanInputMessage:
               ...
               {
                  KernelEventsManager.getInstance().processCallback(BeriliaHookList.MouseClick,MouseClickMessage(param1).target);
                  if(!this.hierarchy[this.hierarchy.length - 1])
                  {
                     KernelEventsManager.getInstance().processCallback(BeriliaHookList.FocusChange,MouseClickMessage(param1).target);
                  }
               }
               if(param1 is MouseMiddleClickMessage)
               {
                  KernelEventsManager.getInstance().processCallback(BeriliaHookList.MouseMiddleClick,MouseMiddleClickMessage(param1).target);
               }
               if(param1 is KeyboardKeyDownMessage)
               {
                  _loc12_ = KeyboardKeyDownMessage(param1);
                  KernelEventsManager.getInstance().processCallback(BeriliaHookList.KeyDown,_loc12_.target,_loc12_.keyboardEvent.keyCode);
               }
               if(param1 is KeyboardKeyUpMessage)
               {
                  _loc13_ = KeyboardKeyUpMessage(param1);
                  KernelEventsManager.getInstance().processCallback(BeriliaHookList.KeyUp,_loc13_.target,_loc13_.keyboardEvent.keyCode);
               }
               _loc6_ = false;
               _loc9_ = false;
                  ...
                  if(!_loc6_ || _loc7_)
                  {
                     ...
                     if(_loc15_)
                     {
                        ...
                        else
                        {
                           ...
                           this.currentDo = null;
                           this._isProcessingDirectInteraction = false;
                           ModuleLogger.log(_loc5_,_loc14_);
                           return true;
                        }
                     }
                  }
               ...
               while(this.currentDo != null)
               {
                  ...
                  if(this.currentDo is GraphicContainer)
                  {
                     ModuleLogger.log(param1,this.currentDo);
                  }
                  this.currentDo = this.currentDo.parent;
               }
               if(param1 is MouseClickMessage)
               {
                  KernelEventsManager.getInstance().processCallback(BeriliaHookList.PostMouseClick,MouseClickMessage(param1).target);
               }
                     ...
                     if(UIEventManager.getInstance().instances[this.currentDo] && UIEventManager.getInstance().instances[this.currentDo].events[getQualifiedClassName(param1)])
                     {
                        ...
                        switch(true)
                        {
                           ...
                           case param1 is SelectItemMessage:
                              ...
                              _loc22_ = [_loc21_.instance,SelectItemMessage(param1).selectMethod,SelectItemMessage(param1).isNewSelection];
                              break;
                           case param1 is SelectEmptyItemMessage:
                              ...
                              _loc22_ = [_loc21_.instance,SelectEmptyItemMessage(param1).selectMethod];
                              break;
                           case param1 is ItemRollOverMessage:
                              _loc22_ = [_loc21_.instance,ItemRollOverMessage(param1).item];
                              break;
                           case param1 is ItemRollOutMessage:
                              _loc22_ = [_loc21_.instance,ItemRollOutMessage(param1).item];
                              break;
                           case param1 is ItemRightClickMessage:
                              this._isProcessingDirectInteraction = true;
                              _loc22_ = [_loc21_.instance,ItemRightClickMessage(param1).item];
                              break;
                           ...
                           case param1 is DeleteTabMessage:
                              _loc22_ = [_loc21_.instance,DeleteTabMessage(param1).deletedIndex];
                              break;
                           case param1 is RenameTabMessage:
                              _loc22_ = [_loc21_.instance,RenameTabMessage(param1).index,RenameTabMessage(param1).name];
                              break;
                           case param1 is MapElementRollOverMessage:
                              _loc22_ = [_loc21_.instance,MapElementRollOverMessage(param1).targetedElement];
                              break;
                           case param1 is MapElementRollOutMessage:
                              _loc22_ = [_loc21_.instance,MapElementRollOutMessage(param1).targetedElement];
                              break;
                           case param1 is MapElementRightClickMessage:
                              _loc22_ = [_loc21_.instance,MapElementRightClickMessage(param1).targetedElement];
                              break;
                           case param1 is MapMoveMessage:
                              _loc22_ = [_loc21_.instance];
                              break;
                           case param1 is MapRollOverMessage:
                              _loc22_ = [_loc21_.instance,MapRollOverMessage(param1).x,MapRollOverMessage(param1).y];
                              break;
                           case param1 is MapRollOutMessage:
                              _loc22_ = [_loc21_.instance];
                              break;
                           ...
                           case param1 is VideoBufferChangeMessage:
                              _loc22_ = [_loc21_.instance,VideoBufferChangeMessage(param1).state];
                              break;
                           ...
                           case param1 is TextClickMessage:
                              this._isProcessingDirectInteraction = true;
                              _loc22_ = [_loc21_.instance,(param1 as TextClickMessage).textEvent];
                        }
                        if(_loc22_)
                        {
      ...
      private function processRegisteredUiEvent(param1:Message, param2:Grid) : void
      {
         ...
         if(param2)
         {
            ...
            {
               _loc5_ = [_loc3_.instance,MouseWheelMessage(param1).mouseEvent.delta];
            }
            else
            {
               _loc5_ = [_loc3_.instance];
            }
         ...
         {
            ErrorManager.tryFunction(_loc3_.callbackObject[_loc4_],[_loc3_.instance,MouseWheelMessage(param1).mouseEvent.delta]);
         }
         else
         {
            ErrorManager.tryFunction(_loc3_.callbackObject[_loc4_],[_loc3_.instance]);
         }
      }
      ...
      private function onNativeWindowResize(param1:Event = null) : void
      {
