package com.ankamagames.berilia.frames
{
   ...
   public class ShortcutsFrame implements Frame
   {
         ...
         switch(true)
         {
            case param1 is KeyboardKeyDownMessage:
               ...
               if(_loc3_ && _loc3_.holdKeys && this._heldShortcuts.indexOf(_loc3_.defaultBind.targetedShortcut) == -1)
               {
                  ...
                  _loc5_ = StageShareManager.stage.focus as TextField;
                  if(!_loc5_ || (_loc3_.textfieldEnabled || _loc5_.type != TextFieldType.INPUT))
                  {
      ...
      private function handleMessage(param1:KeyboardMessage) : Boolean
      {
         ...
         {
            _loc9_ = _loc5_.copy();
            _loc9_.alt = _loc9_.ctrl = _loc9_.shift = false;
            _loc6_ = BindsManager.getInstance().getBind(_loc9_);
            if(_loc6_ && this._heldShortcuts.indexOf(_loc6_.targetedShortcut) != -1)
            {
               _loc7_ = Shortcut.getShortcutByName(_loc6_.targetedShortcut);
            }
         }
         ...
         if(_loc6_ != null && _loc7_ && !_loc7_.disable)
         {
            ...
            if(_loc7_.holdKeys)
            {
               _loc11_ = this._heldShortcuts.indexOf(_loc7_.defaultBind.targetedShortcut);
               if(_loc11_ != -1)
               {
                  this._heldShortcuts.splice(_loc11_,1);
                  this._lastHeldShortcut = false;
               }
            }
            _loc10_ = StageShareManager.stage.focus as TextField;
            if(_loc10_ && _loc10_.type == TextFieldType.INPUT)
            {
