package com.ankamagames.tiphon.engine
{
   ...
   public class LibrariesManager extends EventDispatcher
   {
      ...
      public function hasAnim(param1:int, param2:String, param3:int = -1) : Boolean
      {
         ...
         var _loc5_:GraphicLibrary = this._aResources[param1];
         if(_loc5_ && _loc5_.isSingleFile)
         {
            _loc6_ = _loc5_.getSwl();
            if(!_loc6_)
            {
               _log.info("On test si une librairie contient une anim sans l\'avoir en mémoire. (bones: " + param1 + ", anim:" + param2 + ")");
               return _loc4_;
            }
            _loc7_ = _loc6_.getDefinitions();
            _loc8_ = param2 + (param3 != -1?"_" + param3:"");
            for each(_loc9_ in _loc7_)
            {
               if(_loc9_.indexOf(_loc8_) == 0)
               {
                  _loc4_ = true;
                  break;
               }
         ...
         {
            _loc4_ = BoneIndexManager.getInstance().hasAnim(param1,param2,param3);
         }
         return _loc4_;
      }
      ...
      private function onLoadResource(param1:ResourceLoadedEvent) : void
      {
         ...
         var _loc2_:int = param1.uri.tag.id == null?int(param1.uri.tag):int(param1.uri.tag.id);
      ...
      private function onLoadFailedResource(param1:ResourceErrorEvent) : void
      {
         ...
         var _loc4_:int = 0;
         var _loc5_:int = 0;
         var _loc2_:int = !!isNaN(param1.uri.tag)?int(param1.uri.tag.id):int(param1.uri.tag);
