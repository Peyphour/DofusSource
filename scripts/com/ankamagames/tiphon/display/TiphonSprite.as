package com.ankamagames.tiphon.display
{
   ...
   public class TiphonSprite extends Sprite implements IAnimated, IAnimationSpriteHandler, IDestroyable, EntityLookObserver
   {
      ...
      override public function set alpha(param1:Number) : void
      {
         this._alpha = param1;
         var _loc2_:Number = 1;
         var _loc3_:TiphonSprite = this.parentSprite;
         while(_loc3_)
         {
            _loc2_ = _loc2_ * (1 / _loc3_.realAlpha);
            _loc3_ = _loc3_.parentSprite;
         }
         super.alpha = this._alpha * _loc2_;
         var _loc4_:TiphonSprite = (TiphonUtility.getEntityWithoutMount(this) as TiphonSprite).carriedEntity;
         if(_loc4_)
         {
      ...
      {
         var _loc1_:uint = this._look.getBone();
         if(BoneIndexManager.getInstance().hasCustomBone(_loc1_))
         {
            return BoneIndexManager.getInstance().getAllCustomAnimations(_loc1_);
         }
         var _loc2_:Swl = Tiphon.skullLibrary.getResourceById(_loc1_);
         return !!_loc2_?_loc2_.getDefinitions():null;
      }
      ...
      public function setAnimationAndDirection(param1:String, param2:uint, param3:Boolean = false) : void
      {
         ...
         var _loc10_:IAnimationModifier = null;
         var _loc11_:TiphonEvent = null;
         ...
         if(this is IEntity)
         {
            if(this._currentAnimation && (this._currentAnimation.indexOf("AnimMarche") == 0 || this._currentAnimation.indexOf("AnimCourse") == 0) && param1 == "AnimStatique")
            {
         ...
         {
            _loc9_ = _loc4_.animation;
            for each(_loc10_ in this._animationModifier)
            {
               _loc4_.animation = _loc10_.getModifiedAnimation(_loc9_,this.look);
            }
         }
         if(param3)
         {
         ...
         if(!this.overrideNextAnimation && _loc4_.animation == this._currentAnimation && param2 == this._currentDirection && this._rendered)
         {
            ...
            _loc11_ = new TiphonEvent(TiphonEvent.RENDER_SUCCEED,this);
            _loc11_.animationName = this._currentAnimation + "_" + this._currentDirection;
         ...
         if(!param3)
         {
            ...
            {
               _loc12_ = BoneIndexManager.getInstance().getTransition(this._look.getBone(),this._lastAnimation,_loc4_.animation,this._currentDirection);
               this._currentAnimation = _loc12_;
               this._transitionStartAnimation = _loc12_;
               this._transitionEndAnimation = _loc4_.animation;
            }
            else
            {
         ...
         {
            _loc13_ = BoneIndexManager.getInstance().getBoneFile(this._look.getBone(),this._currentAnimation);
            if(_loc13_.fileName != this._look.getBone() + ".swl" || BoneIndexManager.getInstance().hasAnim(this._look.getBone(),this._currentAnimation,this._currentDirection))
            {
               this.initializeLibrary(this._look.getBone(),_loc13_);
            }
            else
            {
      ...
      public function hasAnimation(param1:String, param2:int = -1) : Boolean
      {
         ...
         else
         {
            ...
            while(_loc5_ < 8)
            {
               if(Tiphon.skullLibrary.hasAnim(_loc4_,param1,_loc5_))
               {
                  _loc3_ = true;
                  break;
               }
         ...
         if(!_loc3_)
         {
            _loc6_ = this._look.getSubEntity(2,0);
            if(_loc6_)
            {
               _loc4_ = _loc6_.getBone();
               if(param2 != -1)
               {
                  _loc3_ = Tiphon.skullLibrary.hasAnim(_loc4_,param1,param2) || Tiphon.skullLibrary.hasAnim(_loc4_,param1,TiphonUtility.getFlipDirection(param2));
               }
      ...
      override public function dispatchEvent(param1:Event) : Boolean
      {
         ...
         if(param1.type == TiphonEvent.ANIMATION_END && this._transitionEndAnimation)
         {
            if(this._transitionStartAnimation == this._currentAnimation)
            {
               super.dispatchEvent(new TiphonEvent(TiphonEvent.ANIMATION_TRANSITION_END,(param1 as TiphonEvent).sprite,(param1 as TiphonEvent).params));
               _loc2_ = this._transitionEndAnimation;
      ...
      private function checkRenderState() : void
      {
         ...
         {
            this._changeDispatched = true;
            this._timeoutId = setTimeout(this.dispatchEvent,1,_loc2_);
         }
      }
      ...
      public function onAnimationEvent(param1:String, param2:String = "") : void
      {
         ...
         {
            this.parentSprite.onAnimationEvent(param1,param2);
         }
      }
