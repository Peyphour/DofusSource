package com.ankamagames.jerakine.messages
{
   ...
   public class Worker extends EventDispatcher implements MessageHandler
   {
      ...
      private function processMessages() : void
      {
         ...
         var _loc9_:Vector.<Message> = null;
         var _loc10_:Message = null;
         if(this._messagesQueue.length > MAX_MESSAGES_PER_FRAME)
         {
            ...
            _loc7_ = 0;
            while(_loc7_ < this._messagesQueue.length)
            {
               _loc8_ = getQualifiedClassName(this._messagesQueue[_loc7_]);
               if(this._blacklistedFloodMessages[_loc8_.split("::")[1]])
               {
                  if(!_loc4_[_loc8_])
                  {
                     _loc4_[_loc8_] = 1;
                  }
                  else if(_loc4_[_loc8_]++ > _loc3_)
                  {
                     _loc6_[_loc8_] = true;
                     _loc5_ = true;
                  }
            ...
            if(_loc5_)
            {
               _loc9_ = new Vector.<Message>();
               _loc7_ = 0;
               while(_loc7_ < this._messagesQueue.length)
               {
                  _loc8_ = getQualifiedClassName(this._messagesQueue[_loc7_]);
                  _loc4_[_loc8_] = _loc4_[_loc8_] - 1;
                  if(!_loc6_[_loc8_] || _loc4_[_loc8_] == 0)
                  {
                     _loc9_.push(this._messagesQueue[_loc7_]);
                  }
                  _loc7_++;
               }
               this._messagesQueue = _loc9_;
            }
         }
         var _loc1_:uint = 0;
         ...
         {
            _loc10_ = this._messagesQueue.shift();
            if(!(_loc10_ is CancelableMessage && CancelableMessage(_loc10_).cancel))
            {
               if(this._paused && _loc10_ is QueueableMessage && !this.msgIsUnstoppable(_loc10_))
               {
                  this._pausedQueue.push(_loc10_);
                  _log.warn("Queued message: " + _loc10_);
               }
               else
               {
                  this.processMessage(_loc10_);
                  if(_loc10_ is Poolable)
                  {
                     GenericPool.free(_loc10_ as Poolable);
                  }
                  this.processFramesInAndOut();
                  _loc1_++;
               }
