package com.ankamagames.jerakine.utils.benchmark.monitoring.ui
{
   ...
   public class GraphDisplayer extends Sprite
   {
      ...
      public function update(param1:Boolean = true) : void
      {
         ...
         for each(_loc5_ in this._graphToDisplay)
         {
            ...
            if(!(_loc5_.points.length == 0 || !_loc5_.graphVisible))
            {
               ...
               _loc4_ = FpsManagerUtils.addAlphaToColor(_loc5_.color,4294967295);
               ...
               this._graphDisplay.bitmapData.setPixel32(_loc2_,_loc3_,_loc3_ == 1?uint(4294901760):uint(_loc4_));
            }
         }
      ...
      private function showGraph(param1:Event) : void
      {
         ...
         var _loc4_:int = 0;
         var _loc7_:uint = FpsManagerUtils.addAlphaToColor(_loc2_.color,4294967295);
         this._graphDisplay.bitmapData.lock();
         _loc4_ = 0;
         while(_loc4_ < _loc3_)
         {
            _loc5_ = _loc3_ < FpsManagerConst.BOX_WIDTH?int(FpsManagerConst.BOX_WIDTH - _loc3_ + _loc4_):int(_loc4_);
